...
# - Configures static IPv4 addressing (netplan or systemd-networkd)
...
# ----------------------------------------------------------
# PROMPT STATIC IPv4 (interactive)
# ----------------------------------------------------------
prompt_ipv4() {
    step "STATIC IPv4 SETUP"

    # Show current interfaces
    info "Detected network interfaces:"
    ip -4 -brief addr show | grep -v '^lo ' || true
    echo ""

    echo -e "${CYAN}${BOLD}Enter the interface to configure with a static IPv4 address.${NC}"
    echo -e "${YELLOW}Press Enter to SKIP (keep current network config).${NC}"
    echo ""

    read -rp "Interface (e.g. eth0, ens18): " NETWORK_INTERFACE
    if [[ -z "$NETWORK_INTERFACE" ]]; then
        warn "No interface provided — skipping static IPv4 setup"
        return
    fi

    # Verify interface exists
    if ! ip link show "$NETWORK_INTERFACE" &>/dev/null; then
        warn "Interface '$NETWORK_INTERFACE' not found — skipping"
        NETWORK_INTERFACE=""
        return
    fi
    ...
    read -rp "IPv4 address (e.g. 192.168.1.100): " IPV4_ADDRESS
    if [[ -z "$IPV4_ADDRESS" ]]; then
        warn "No address provided — skipping static IPv4 setup"
        NETWORK_INTERFACE=""
        return
    fi
    if ! [[ "$IPV4_ADDRESS" =~ $ipv4_re ]]; then
        warn "Address format not recognized — proceeding anyway"
    fi

    read -rp "Netmask in CIDR notation [24]: " IPV4_NETMASK
    IPV4_NETMASK="${IPV4_NETMASK:-24}"

    read -rp "Gateway (e.g. 192.168.1.1): " IPV4_GATEWAY
    if [[ -z "$IPV4_GATEWAY" ]]; then
        warn "No gateway provided — routing may not work"
    elif ! [[ "$IPV4_GATEWAY" =~ $ipv4_re ]]; then
        warn "Gateway format not recognized — proceeding anyway"
    fi

    read -rp "DNS servers, comma-separated [1.1.1.1,8.8.8.8]: " IPV4_DNS
    IPV4_DNS="${IPV4_DNS:-1.1.1.1,8.8.8.8}"

    echo ""
    info "Will configure $NETWORK_INTERFACE: ${IPV4_ADDRESS}/${IPV4_NETMASK} gw ${IPV4_GATEWAY} dns ${IPV4_DNS}"
}
...
install_essentials() {
    ...
    safe_install fastfetch || install_fastfetch_github
...
# ----------------------------------------------------------
# FASTFETCH FALLBACK (not in Debian 12 / Ubuntu 24.04 repos)
# ----------------------------------------------------------
install_fastfetch_github() {
    info "fastfetch not in apt repos — installing from GitHub releases"
    ...
    # Get the latest .deb URL for our architecture
    local deb_url
    deb_url=$(curl -fsSL "$api_url" \
        | grep -oP '"browser_download_url":\s*"\K[^"]+linux-'"$arch"'\.deb(?=")' \
        | head -1)

    if [[ -z "$deb_url" ]]; then
        warn "Could not find fastfetch .deb for architecture $arch"
        return 1
    fi

    if curl -fsSL "$deb_url" -o "$tmp_deb"; then
        dpkg -i "$tmp_deb" >> "$LOG_FILE" 2>&1 || apt-get install -f -y >> "$LOG_FILE" 2>&1
        rm -f "$tmp_deb"
        if command -v fastfetch &>/dev/null; then
            success "Installed: fastfetch (from GitHub)"
        else
            warn "fastfetch .deb installed but binary not found"
            return 1
        fi
    else
...
setup_console_font() {
    ...
    if [[ -f /etc/default/console-setup ]]; then
        ...
        # setupcon fails with ioctl errors when no physical TTY is available (VMs/SSH)
        if [[ -t 0 ]] && [[ -c /dev/console ]]; then
            setupcon --force >> "$LOG_FILE" 2>&1 || true
        fi
        success "Console font set to Terminus 16x32 (applied on next TTY login)"
    else
...
# ----------------------------------------------------------
# STATIC IPv4 CONFIGURATION (netplan or systemd-networkd)
# ----------------------------------------------------------
setup_static_ipv4() {
    step "APPLYING STATIC IPv4 CONFIGURATION"

    if [[ -z "$NETWORK_INTERFACE" ]]; then
        info "No static IPv4 requested — skipping"
        return
    fi
    ...
    if command -v netplan &>/dev/null; then
        ...
        warn "Neither netplan nor systemd-networkd found — cannot configure network"
        return
    fi

    info "Detected network backend: $NETWORK_BACKEND"

    # Build DNS list (space-separated for netplan, semicolon for networkd)
    local dns_spaces dns_semicolons
    dns_spaces="${IPV4_DNS//,/ }"
    dns_semicolons="${IPV4_DNS//,/;}"

    if [[ "$NETWORK_BACKEND" == "netplan" ]]; then
        # Backup existing netplan configs
        local backup_dir="/etc/netplan/backup_$(date +%Y%m%d_%H%M%S)"
        mkdir -p "$backup_dir"
        cp -p /etc/netplan/*.yaml "$backup_dir/" 2>/dev/null || true
        info "Existing netplan configs backed up to $backup_dir"

        # Remove old configs so the new static config replaces (not coexists with) DHCP/old IPs
        rm -f /etc/netplan/*.yaml 2>/dev/null || true

        cat << EOF > /etc/netplan/60-debuntu-static.yaml
# Generated by Debuntu provisioning script — $(date '+%Y-%m-%d %H:%M:%S')
network:
  version: 2
  ethernets:
    ${NETWORK_INTERFACE}:
      ...
      routes:
        - to: default
          via: ${IPV4_GATEWAY}
      nameservers:
        addresses: [${dns_spaces// /, }]
      dhcp4: false
EOF

        chmod 600 /etc/netplan/60-debuntu-static.yaml
        success "Netplan config written to /etc/netplan/60-debuntu-static.yaml"

    elif [[ "$NETWORK_BACKEND" == "networkd" ]]; then
        local net_file="/etc/systemd/network/60-debuntu-static.network"
        local backup_dir="/etc/systemd/network/backup_$(date +%Y%m%d_%H%M%S)"
        mkdir -p "$backup_dir"

        # Backup and remove existing networkd configs for this interface to replace (not add to) the current IP
        for f in /etc/systemd/network/*.network; do
            [[ -f "$f" ]] || continue
            [[ "$f" == "$net_file" ]] && continue
            if grep -q "Name=${NETWORK_INTERFACE}" "$f" 2>/dev/null; then
                mv "$f" "$backup_dir/"
            fi
        done
        info "Existing networkd configs backed up to $backup_dir"
...
EOF
        ...
        systemctl enable systemd-networkd >> "$LOG_FILE" 2>&1 || true
        success "systemd-networkd config written to $net_file"
    fi

    warn "Network config written but NOT applied (to avoid disconnection)"
    warn "You must apply it manually after the script finishes (see summary)"
}
...
show_summary() {
    ...
    if [[ -n "$NETWORK_INTERFACE" ]]; then
        echo "  - Static IPv4 configured: ${IPV4_ADDRESS}/${IPV4_NETMASK} on ${NETWORK_INTERFACE} (${NETWORK_BACKEND})"
    else
        echo "  - Static IPv4 skipped (no interface specified)"
    fi
    if [[ -n "$SSH_KEY" ]]; then
    ...
    if [[ -n "$NETWORK_INTERFACE" ]]; then
        echo -e "${YELLOW}${BOLD}IMPORTANT: Static IPv4 config was written but NOT applied.${NC}"
        echo "  To apply now (this WILL disconnect you if connected via SSH):"
        if [[ "$NETWORK_BACKEND" == "netplan" ]]; then
...
main() {
    ...
    prompt_ipv4                  # interactive static IPv4 input
    ...
    setup_fastfetch
    setup_ufw
    setup_static_ipv4            # apply static IPv4 if requested
